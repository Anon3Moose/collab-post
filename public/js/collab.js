$(document).ready(function() {
  ...
  function upsertCollab(collabData) {
    $.post("/api/collabs", collabData).then(getCollabs);
  }
  ...
  function createCollabRow(collabData) {
    ...
    newTr.append(
      "<td><a href='/blog?collab_id=" + collabData.id + "'>Go to Posts</a></td>"
    );
    newTr.append(
      "<td><a href='/cms?collab_id=" +
        collabData.id +
        "'>Create a Post</a></td>"
    );
    newTr.append(
      "<td><a style='cursor:pointer;color:red' class='delete-collab'>Delete Collab</a></td>"
    );
    return newTr;
  }
